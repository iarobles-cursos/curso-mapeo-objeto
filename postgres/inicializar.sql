...
CREATE TABLE datos_articulos(
   id INT DEFAULT nextval('datos_articulos_id_seq'),
   observaciones TEXT,
   PRIMARY KEY(id)   
);
...
CREATE TABLE domicilios(
   id INT DEFAULT nextval('domicilios_id_seq'),
   usuario_id INT,
   calle VARCHAR(300),
   numero VARCHAR(100),
   colonia VARCHAR(300),
...
CREATE TABLE ratings(
    id INT DEFAULT nextval('ratings_id_seq'),
    rating INT,
    articulo_id INT,
    PRIMARY KEY(id),
    FOREIGN KEY(articulo_id)  REFERENCES articulos
...
CREATE TABLE telefonos(
  id INT DEFAULT nextval('telefonos_id_seq'),
  usuario_id INT,
...
CREATE TABLE emails_adicionales(
  id INT DEFAULT nextval('emails_adicionales_id_seq'),
  usuario_id INT,
  email VARCHAR(300),
  PRIMARY KEY(id),
  FOREIGN KEY(usuario_id) REFERENCES usuarios
